...
const BASE = `${API_URL}/api/v1/admin/settings/assets`
...
export async function updateAssetCategory(id, payload) {
  const { data } = await client.put(`/categories/${encodeURIComponent(id)}`, payload)
  return data
}

export async function deleteAssetCategory(id) {
  const { data } = await client.delete(`/categories/${encodeURIComponent(id)}`)
  return data
}
