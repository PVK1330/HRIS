...
const BASE = `${API_URL}/api/v1/admin/settings/payroll`
...
function unwrapError(err) {
  const apiMsg = err?.response?.data?.message || err?.response?.data?.error || err?.message || 'Request failed'
...
export async function fetchPayrollPolicySettings() {
  const { data } = await client.get('/')
  return data?.data ?? data
}

export async function updatePayrollPolicySettings(payload) {
