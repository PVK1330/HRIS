  ...
  const fetchRoles = async () => {
    try {
      ...
      setRoles(list.map((role) => ({
        ...
        icon: role.role_key === 'superadmin' ? HiFingerPrint : role.role_key === 'billing_admin' ? HiCreditCard : HiUserGroup,
        color: role.role_key === 'superadmin' ? 'rose' : role.role_key === 'billing_admin' ? 'emerald' : 'blue',
        ...
        {isLoading ? <div className="text-sm text-slate-500 px-2">Loading roles...</div> : roles.map((role) => (
          <div key={role.id} className="flex flex-col rounded-xl border border-slate-100 bg-white shadow-sm overflow-hidden transition-all hover:shadow-md group">
            ...
            <div className={`p-5 border-b border-slate-50 relative`}>
              ...
              <div className={`h-14 w-14 rounded-2xl flex items-center justify-center mb-6 shadow-sm border border-slate-50 transition-transform group-hover:scale-110 ${role.color === 'rose' ? 'bg-rose-50 text-rose-600' :
